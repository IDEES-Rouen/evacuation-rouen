...
global {
	...
	file shape_file_roads  <- file("../includes/roads_7200_pm_ok_v1.shp") ;
	file shape_file_nodes  <- file("../includes/nodes_7200_pm_ok_v1.shp");
	...
	string shortest_paths_file1 <- "shortest_paths_speed_lanes_3.csv";
	string shortest_paths_file2 <- "shortest_paths_speed_3.csv";
	string shortest_paths_file3 <- "shortest_paths_distance_3.csv";
	string shortest_paths_file4 <- "shortest_paths_traffic_light_3.csv";
	...
	init {
		...
		/*map poids <- road as_map (each::(each.shape.perimeter / each.maxspeed / each.lanes * (each.danger_area ?  100 : 1) ));
		...
		map poids <- road as_map (each::(each.shape.perimeter / each.maxspeed * (each.danger_area ?  100 : 1) ));
		road_graph <-  use_cache(as_driving_graph(road, node_)  with_weights (poids), false) with_optimizer_type "static";
	...
	/*	map poids <- road as_map (each::(each.shape.perimeter * (each.danger_area ?  100 : 1)));
	...
	/* 	list<road> traffic_signal_road <- (road where (node_(each.target_node).is_traffic_signal));
		...
		poids <-  road as_map (each::(each.shape.perimeter / each.maxspeed* ((each in traffic_signal_road) ? 1000 : 1) * (each.danger_area ?  100 : 1)));
		road_graph <-  use_cache(road_graph with_weights (poids), false) with_optimizer_type "static";
