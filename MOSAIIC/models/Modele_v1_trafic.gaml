...
global {   
	file shape_file_roads  <- file("../includes/data_validation/routes_simplifieees_lanes_pont_guillaume_ok.shp") ;
	...
	float proportion_speed <- 0.0;
	float proportion_distance <-0.0;
	...
	init {  
		...
		create road from: shape_file_roads with:[name::string(read("name")),highway::string(get("highway")),junction::string(read("junction")),lanes::int(read("lanes")), maxspeed::float(read("maxspeed")), oneway::string(read("oneway")), lanes_forward ::int (get( "lanesforwa")), lanes_backward :: int (get("lanesbackw"))] {
			if lanes = 0 {lanes <- 1;}
			if maxspeed = 0 {maxspeed <- 30#km/#h;}
	...
	action create_people {
		...
		//	nb_people_computed <- nb_people_computed + 1;
			...
			proba_lane_change_down <- 0.7+ (rnd(300) / 500);
			bool ok <- false;
			loop while: not ok {
	...
	action init_boucle {
		...
		ask boucle {
			...
			if  (name = "" or (not (world overlaps self))) {do die;} 
			else {
				...
				if (empty(roads_close)) {do die;}
					...
					data_2012 <- file_exists("../includes/_boucles_test_11_octobre_2012/" + name+ ".csv");
					data_2013 <-  file_exists("../includes/_boucles_test_10_octobre_2013/" + name+ ".csv");
					if (data_2012 or data_2013) {
						if (data_2012) {
							...
							loop i from: 1 to: nb_rows - 1 {
								if (not save_data) {
									string date_ <- string(data_boucle_2012[0,i]);
									if date_ = nil or date_ ="" {break;}
									
									list<string> dateL <- date_ split_with ":";
									int h_t <- int(dateL[0]);
									int m_t <- int(dateL[1]);
						...
						if (data_2013) {
							...
							loop i from: 1 to: nb_rows - 1 {
								if (not save_data) {
									string date_ <- string(data_boucle_2013[0,i]);
									if date_ = nil or date_ ="" {break;}
									list<string> dateL <- date_ split_with ":";
									int h_t <- int(dateL[0]);
									int m_t <- int(dateL[1]);
	...
	reflex general_dynamic {
		...
		if (compute_global_error and (time = (time_end - 1))){
			ask road {
				save (string(int(self))+","+ name +","+highway+ ","+maxspeed +","+lanes+","+ shape.perimeter+","+ traffic_jam + "," + nb_pass78 +","+ nb_pass89 + "," + (nb_people_tot = 0 ? 0 : (temps_tot_global/nb_people_tot))) to: ("routes.csv")  rewrite: false;
			}
			ask boucle {
	...
	action store_data {
		...
		if (every(time_step)) {
			...
			if (time >= 1#h and time < 2#h) {
				comptage_78_S1 <- comptage_78_S1 + simulated_value_S1;
				comptage_78_S2 <- comptage_78_S2 + simulated_value_S2;
			} else if (time >= 2#h) {
				comptage_89_S1 <- comptage_89_S1 + simulated_value_S1;
	...
	//	float t <- machine_time;
		...
		if (current_path != nil) {
			...
			if (time >= 1#h and time < 2#h) {
				...
				new_road.nb_pass89 <- new_road.nb_pass89 + 1;
			}
		}
